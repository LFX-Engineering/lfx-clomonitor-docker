...
exclude_pattern='node_modules|.venv|organization-service.yaml|project-service.yaml|acs-service.yaml|user-service.yaml|api.*.compiled.yaml|.vendor-new|.pytest_cache|.idea|cncf.yaml|lfai.yaml'
...
echo "Searching Dockerfile files..."
files+=($(find . -type f -name 'Dockerfile' -print | egrep -v ${exclude_pattern}))
